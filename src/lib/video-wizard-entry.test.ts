...
import {
  initialWizardStep, hydrateSeededWizard, ensureStepInvariant,
  acceptVideoPhotos, attachUploadAssets,
} from "./video-upload-intake";

const mkFile = (n: string) => new File([new Uint8Array([1, 2, 3])], n, { type: "image/jpeg" });
const selectUploads = (w: any) => { w.scenes = (w.gridOrder || []).map((k: string) => ({ key: k })); };
const blank = (over: any = {}) => ({ step: 1, uploads: [], scenes: [], available: [], gridOrder: [], ...over });
...
function deps(w: any, render = vi.fn()) {
  return {
    ...
    advance: async (x: any) => { x.step = 2; attachUploadAssets(x); selectUploads(x); render(); },
    loadAssets: async () => {},
    isCurrent: (x: any) => x === w,
...
describe("seeded uploads (Studio / Create Media handoff)", () => {
  it("hydrates to step 2 with all photos selected before first paint", () => {
    const w: any = blank({ uploads: Array.from({ length: 6 }, (_, i) => ({ id: "s" + i, url: "blob:" + i })) });
...
describe("acceptVideoPhotos", () => {
  it("advances a direct picker upload of six files to step 2", async () => {
    const w: any = blank();
    const render = vi.fn();
    await acceptVideoPhotos({ wizard: w, files: Array.from({ length: 6 }, (_, i) => mkFile(`p${i}.jpg`)), source: "picker", deps: deps(w, render) as any });
    expect(w.step).toBe(2);
  ...
  it("appends from step 2 and stays on step 2 with new photos selected", async () => {
    ...
    attachUploadAssets(w); selectUploads(w);
    await acceptVideoPhotos({ wizard: w, files: [mkFile("n1.jpg"), mkFile("n2.jpg")], source: "step2", deps: deps(w) as any });
    expect(w.step).toBe(2);
