...
const stage = (kind: BucketStage["kind"], bucket = "room-photos"): BucketStage => ({ bucket, kind, detail: "" });
...
describe("storage health stages", () => {
  ...
  it("separates a missing bucket from a policy failure", () => {
    ...
    expect(classifyStorageError("new row violates row-level security policy", "upload_failure")).toBe("policy_failure");
  ...
  it("reports the exact access rules a bucket lacks", () => {
    ...
    expect(missingCommands("reveal-videos", rows)).toEqual(["SELECT", "INSERT", "UPDATE", "DELETE"]);
  });
  ...
  it("keeps the end-user sentence neutral and stage-appropriate", () => {
    ...
    expect(userMessageFor([stage("bucket_missing")])).toMatch(/Uploads are temporarily unavailable/);
