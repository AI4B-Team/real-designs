...
export const SERVER_CONFIG: ConfigVar[] = [
  { name: "SUPABASE_URL", required: true, purpose: "Backend API endpoint used by server functions.", secret: false },
  { name: "SUPABASE_SERVICE_ROLE_KEY", required: true, purpose: "Privileged backend key for admin-only server work.", secret: true },
  { name: "SUPABASE_PUBLISHABLE_KEY", required: false, purpose: "Public backend key for server-side anonymous reads.", secret: false },
  { name: "SUPABASE_ANON_KEY", required: false, purpose: "Legacy alias of the publishable key.", secret: false },
  { name: "LOVABLE_API_KEY", required: true, purpose: "AI gateway access for design, scoring and video features.", secret: true },
];

export const CLIENT_CONFIG: ConfigVar[] = [
  { name: "VITE_SUPABASE_URL", required: true, purpose: "Backend endpoint for the browser client.", secret: false },
  { name: "VITE_SUPABASE_PUBLISHABLE_KEY", required: true, purpose: "Public backend key for the browser client.", secret: false },
];
...
export function validateServerConfig(env: Record<string, string | undefined>): ConfigReport {
  const missing = SERVER_CONFIG.filter((v) => v.required && !env[v.name]?.trim()).map((v) => v.name);
  ...
  if (!env["SUPABASE_PUBLISHABLE_KEY"] && !env["SUPABASE_ANON_KEY"]) {
    warnings.push("No publishable key is set; server-side anonymous reads will fall back to the admin client.");
  }
...
export function describeConfigReport(report: ConfigReport): string {
  ...
  if (report.leaked.length) parts.push(`secret exposed to the browser: ${report.leaked.join(", ")}`);
