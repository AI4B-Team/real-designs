...
describe("effects modal view model", () => {
  it("only lists categories that hold options, All first", () => {
    ...
    lookCats()
      .slice(1)
      .forEach(([id]) => expect(looksForCat(id).length).toBeGreaterThan(0));
    fxCats()
      .slice(1)
      .forEach(([id]) => expect(effectTiles(id).length).toBeGreaterThan(0));
  });
  ...
  it("shows intensity only for graded scenes and disclosure only for generated ones", () => {
    ...
    expect(needsDisclosure(scene({ vfx: "virtual_staging", vfx_gen: "virtual_staging" }))).toBe(
      true,
    );
  ...
  it("prices paid effects per scene and only bills scenes that change", () => {
    ...
    expect(applyAllPlan(scenes, src)).toMatchObject({
      total: 3,
      targets: 2,
      perScene: 6,
      credits: 12,
