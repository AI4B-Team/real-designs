 ...
 */
export function normalizeFormats(formats: unknown): {
  primaryFormat: VideoFormat;
  additionalFormats: VideoFormat[];
} {
  const list = (Array.isArray(formats) ? formats : [])
    .map((f) => (f === "4:5" ? "9:16" : f))
    .filter(isVideoFormat) as VideoFormat[];
  const primaryFormat = list[0] || DEFAULT_FORMAT;
  const additionalFormats = list
    .slice(1)
    .filter((f, i, a) => f !== primaryFormat && a.indexOf(f) === i);
...
export const QUALITY_TIERS: QualityTier[] = [
  ...
  },
  {
    id: "high",
    name: "High",
    note: "1080p, Sharper Motion And Longer Cut",
    maxScenes: 30,
    costMultiplier: 2,
  },
  {
    id: "ultra",
    name: "Ultra",
    note: "4K Master, Every Photo Used",
    maxScenes: 60,
    costMultiplier: 3,
  },
];
