/**
 * The one presentation report renderer.
 *
 * Both the client link (/pkg/$token) and the owner preview
 * (/presentation/$id/preview) render this component, so the owner always sees
 * exactly what the recipient sees. Preview mode is read-only: no comments, no
 * decision, and nothing is recorded against the presentation's analytics.
 */
...
import { commentOnPackage, decideOnPackage } from "@/lib/presentation-packages.functions";
...
export function PackageReport({
  ...
  /** Share token; absent in owner preview. */
  token?: string;
  pack: any;
  preview?: boolean;
}) {
  ...
  async function unlock() {
    ...
    setBusy(true);
    try {
      const { getSharedPackage } = await import("@/lib/presentation-packages.functions");
  ...
  const banner = preview ? <PreviewBanner /> : null;

  if (!gate)
    return (
      <>
        {banner}
        <Shell>{gateMessage(null)}</Shell>
      </>
    );
  ...
  if (gate.error)
    return (
      <>
        {banner}
        <Shell>{gateMessage(gate.error)}</Shell>
      </>
    );
      ...
      <main className="sp-wrap" style={{ "--sp-accent": accent } as Record<string, string>}>
        ...
        {items.length === 0 ? (
          <section className="sp-card sp-center">
            <p>This presentation is temporarily unavailable.</p>
            <p className="sp-sub">
              {preview
                ? "Add at least one design before sharing. Your client would see this message."
                : "The sender has been notified. Check back shortly."}
            </p>
          </section>
        ) : (
          <>
              ...
              <div className="sp-film">
                {items.map((it, i) => (
                  <button
                    key={it.id}
                    className={
                      perms.mode === "slideshow" && i === cursor ? "sp-thumb on" : "sp-thumb"
                    }
            ...
            {shown.map((it) => (
              <ItemView
                ...
                total={items.length}
                comments={comments}
                canComment={interactive && perms.comments}
        ...
        {items.length > 0 && (perms.approve || perms.changes) ? (
          <section className="sp-card" id="decision">
            {!interactive ? (
              <>
                <h2 className="sp-h2">Your Client's Decision</h2>
                <p className="sp-sub">
                  Your client can{" "}
                  {[perms.approve ? "approve this presentation" : null, perms.changes ? "request changes" : null]
                    .filter(Boolean)
                    .join(" or ")}
                  {perms.comments ? " and leave comments" : ""}. Those actions are disabled in
                  preview.
                </p>
              </>
            ) : decision ? (
        ...
        <footer className="sp-foot">
          Shared securely through REAL DESIGNS. No account needed.
        </footer>
      </main>
    </>
  );
...
function PreviewBanner() {
  return (
    <div className="sp-preview-bar" role="status">
      <span>
