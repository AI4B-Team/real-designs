...
export function initSite(): () => void {
  ...
  <g font-family="DM Mono, monospace" font-size="12">
    ...
    const TOUR = [
      {
        ch: "Exterior",
        dur: 1800,
        img: PHOTOS.exteriorBefore,
        style: "As Found",
        lock: "Reality Lock On",
        lab: "Space",
        est: "Front Elevation",
        proc: { msg: "Reading Elevation" },
        mets: [
          ["Style Match", "Calculating", "neutral"],
          ["Reality Lock", "On", "positive"],
          ["Status", "Reading Elevation", "processing"],
        ],
      },
      {
        ch: "Exterior",
        ...
        fit: "Within Target",
        mets: [
          ["Style Match", "96%", "positive"],
          ["Reality Lock", "On", "positive"],
          ["Structure", "No Changes", "positive"],
        ],
      },
      {
        ch: "Exterior",
      ...
      },
      {
        ...
        fit: "Approved",
        mets: [
          ["Style Match", "97%", "positive"],
          ["Reality Lock", "Approved", "positive"],
          ["Structure", "No Changes", "positive"],
        ],
        note: "Stepping Inside",
      },

      {
        ...
        lab: "Room",
        est: "Living Room",
        proc: { msg: "Reading Space" },
        mets: [
          ["Style Match", "Calculating", "neutral"],
          ["Reality Lock", "On", "positive"],
          ["Status", "Reading Space", "processing"],
        ],
      },
      {
        ch: "Interior",
        ...
        fit: "Within Target",
        mets: [
          ["Style Match", "95%", "positive"],
          ["Layout", "High Confidence", "neutral"],
          ["Structure", "No Changes", "positive"],
        ],
      },

      {
        ...
        lab: "Room",
        est: "Living Room",
        proc: { msg: "Detecting Contents" },
        mets: [
          ["Objects Found", "14", "neutral"],
          ["Architecture", "Preserved", "positive"],
          ["Status", "Detecting Contents", "processing"],
        ],
      },
      {
        ch: "Empty Room",
        ...
        lab: "Room",
        est: "Living Room",
        fit: "Architecture Preserved",
        mets: [
          ["Objects Removed", "14 of 14", "neutral"],
          ["Architecture", "Preserved", "positive"],
          ["Disclosure", "Ready", "positive"],
        ],
      },
...

      {
        ...
        shop: true,
        lab: "Project Cart",
        est: "14 Items Matched",
        fit: "Within Target",
        mets: [
          ["Products Matched", "14 of 14", "neutral"],
          ["Fit Checks", "Passed", "positive"],
          ["Confidence", "High", "positive"],
        ],
...

      {
        ch: "Landscape",
        dur: 1600,
        img: PHOTOS.yardBefore,
        style: "As Found",
        lock: "Reality Lock On",
        zoom: "out",
        lab: "Area",
        est: "Backyard",
        proc: { msg: "Reading Site" },
        mets: [
          ["Style Match", "Calculating", "neutral"],
          ["Trades", "3 Detected", "neutral"],
          ["Status", "Reading Site", "processing"],
        ],
        note: "Heading Out Back",
      },
      {
        ch: "Landscape",
        dur: 2500,
        ...
        fit: "Within Target",
        mets: [
          ["Style Match", "92%", "positive"],
          ["Coverage", "Backyard Complete", "positive"],
          ["Confidence", "High", "positive"],
        ],
      },

      {
        ...
        fit: "Ready",
        mets: [
          ["Resolution", "1080p", "neutral"],
          ["Format", "Cinematic Push", "neutral"],
          ["Delivery", "Ready", "positive"],
        ],
      },
      {
        ch: "3D Plan",
        ...
        img: PHOTOS.plan3d,
        style: "Furnished 3D",
        lock: "Design DNA Applied",
        zoom: "aerial",
        lab: "Whole Property",
        est: "4 Rooms Planned",
        fit: "Design DNA Applied",
        mets: [
          ["Rooms Designed", "4 of 4", "neutral"],
          ["Layout", "Preserved", "positive"],
          ["Design DNA", "Applied", "positive"],
        ],
      },

      {
        ...
        dur: 3200,
        img: PHOTOS.resortYard,
        style: "Design DNA Applied",
        lock: "Reality Lock On",
        summary: true,
        lab: "Whole Property",
        est: "4 Rooms Approved",
        fit: "4 Rooms Approved",
        mets: [
          ["Rooms Approved", "4 of 4", "neutral"],
          ["Layout", "Preserved", "positive"],
    ...
    const showStage = document.getElementById("showStage"),
      ...
      styleChip = document.getElementById("styleChip"),
      toastEl = document.getElementById("toast"),
      showNav = document.getElementById("showNav"),
      tourProg = document.getElementById("tourProg");

    showNav.innerHTML = CHAPTERS.map(
      (c, i) => `<button data-ch="${c}"${i === 0 ? ' class="on"' : ""}>${c}</button>`,
    ).join("");
  ...
  <div class="tour-sum">
    ...
    <div class="ts-rows">
      <div><span>Front Elevation</span><em>97% Style Match</em></div>
      <div><span>Living Room</span><em>95% Style Match</em></div>
      <div><span>Furnishings</span><em>14 Items Matched</em></div>
      <div><span>Backyard</span><em>92% Style Match</em></div>
    </div>
    <div class="ts-tot"><span>Rooms Designed</span><em>4 of 4 &middot; One Design DNA</em></div>
  </div>`;
    ...
    function paint(i) {
      ...
      if (showEl) {
        ...
        if (b.summary) {
          ...
          holder.innerHTML = summaryOverlay().trim();
          const node = holder.firstElementChild;
          if (node) showEl.appendChild(node);
        }
      }
      ...
      modePill.innerHTML =
        b.ch === "Empty Room"
          ? "Interior &middot; Empty Room"
          : b.ch === "Virtual Staging"
            ? "Interior &middot; Virtual Staging"
            : b.ch;
      ...
      if (!budgetTouched) {
        ...
        if (hs) {
          const mets = (b.mets || []).map((m, ix) =>
            metric(m[0], m[1], m[2] || (b.proc && ix === 0 ? "processing" : undefined)),
          );
      ...
      if (b.toast)
        tToast = setTimeout(() => {
          toastEl.querySelector("b").textContent = b.toast;
      ...
      showNav
        ...
        .forEach((x) => x.classList.toggle("on", x.dataset.ch === b.ch));
    ...
    showNav.querySelectorAll("button").forEach((btn) =>
   ...
   never shows dollar figures or estimated ranges. */
    ...
    function drawStyles() {
      document.getElementById("styleChips").innerHTML = styleSets[sp]
        .map((s, i) => `<button class="chip ${i === 0 ? "on" : ""}" data-st="${s}">${s}</button>`)
        .join("");
      st = styleSets[sp][0];
      if (PREFILL) {
        var wanted = PREFILL;
        ...
        var hit = styleSets[sp].filter(function (x) {
          return x.toLowerCase() === wanted.toLowerCase();
        })[0];
        if (!hit) {
          var safe = wanted.replace(/[^A-Za-z0-9 &-]/g, "").slice(0, 40);
          if (safe) {
            list.insertAdjacentHTML(
              "afterbegin",
              '<button class="chip" data-st="' + safe + '">' + safe + "</button>",
            );
            hit = safe;
          }
        }
        if (hit) {
          list.querySelectorAll(".chip").forEach(function (c) {
            c.classList.toggle("on", (c.dataset.st || "").toLowerCase() === hit.toLowerCase());
          });
          st = hit;
        }
      }
    ...
    drawStyles();
    document.querySelectorAll("#spaceChips .chip").forEach((c) =>
      c.addEventListener("click", () => {
    ...
    function setBudget(i) {
      ...
      const model = {
        primaryLabel: "Room",
        primaryValue: "Living Room",
        metrics: [
          metric("Design Intensity", b.name, "neutral"),
          metric("What Changes", b.scope),
    ...
    document.querySelectorAll("#budgetChips .chip").forEach((c) =>
      c.addEventListener("click", () => {
    ...
    const steps = [
      ...
      "Locking walls and windows",
      "Matching the design intensity",
   ...
   moment they land, so nothing they entered has to be typed twice. */
    ...
    function saveHandoff() {
      if (!UPLOAD) return;
      const sp =
        (document.querySelector("#spaceChips .chip.on") || { dataset: {} }).dataset.sp ||
        "interior";
      const st = (
        document.querySelector("#styleChips .chip.on") || { textContent: "" }
      ).textContent.trim();
      const payload = {
        ...
        space: sp,
        budget: bi,
        budgetName: BUDGET_NAMES[bi] || "Makeover",
        style: st,
    ...
    document.getElementById("genBtn").addEventListener("click", () => {
      ...
      const out = document.getElementById("out"),
        ov = document.getElementById("genOv"),
        bar = document.getElementById("barFill"),
        gs = document.getElementById("genStep");
      ...
      if (mine) {
        oi.innerHTML =
          '<img src="' +
          UPLOAD.dataUrl +
          '" alt="The space you uploaded" style="width:100%;height:100%;object-fit:cover;display:block">';
      } else {
        oi.innerHTML = room("after", PALS[OUTPAL[si]]);
      }
        ...
        if (p >= 100) {
          ...
          setTimeout(() => {
            if (mine) {
              saveHandoff();
              ov.innerHTML =
                '<div class="gen-gate"><i data-lucide="lock"></i>' +
  ...
  <div class="umodal-card" role="dialog" aria-modal="true" aria-label="Upload your space">
    ...
    modal.querySelectorAll(".usamp").forEach((s, i) => {
      s.innerHTML = room(i === 0 ? "before" : "after", uPals[i]);
    });
    ...
    modal
      ...
      .forEach((el) => el.addEventListener("click", closeUpload));
    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape" && modal.classList.contains("on")) closeUpload();
    });
    ...
    uFile.addEventListener("change", async () => {
      ...
      try {
        UPLOAD = { dataUrl: await shrinkPhoto(f), name: f.name };
        track("upload_photo_selected", { source: "file" });
      } catch (e) {
        UPLOAD = null;
      }
    });
    ...
    let uSel = 0;
    modal.querySelectorAll(".usamp").forEach((s) =>
      s.addEventListener("click", () => {
        modal.querySelectorAll(".usamp").forEach((x) => x.classList.remove("on"));
        s.classList.add("on");
        uSel = +s.dataset.s;
        UPLOAD = null;
        modal.classList.remove("has-file");
      }),
    );
    ...
    /* ---------- marquee ---------- */
    const mqI = [
      ["sofa", "Interior Redesign"],
      ["home", "Exterior Redesign"],
      ["trees", "Landscape Design"],
      ["bed-double", "Virtual Staging"],
      ["eraser", "Declutter Rooms"],
      ["pen-tool", "Sketch to Design"],
      ["paintbrush", "Material Swap"],
      ["ruler", "Style Consistency"],
      ["gauge", "Fast Turnaround"],
      ["shopping-bag", "Shop The Design"],
      ["video", "Walkthrough Video"],
      ["box", "2D to 3D Plans"],
      ["copy", "Style Transfer"],
      ["shield-check", "MLS Disclosure"],
      ["images", "Photo Enhancement"],
    ];
    document.getElementById("mq").innerHTML = [...mqI, ...mqI]
      .map(([i, t]) => `<span class="mq-item"><i data-lucide="${i}"></i>${t}</span>`)
      .join("");

    /* ---------- styles grid ---------- */
    const SL = [
      ["Warm Minimal", PHOTOS.after],
      ["Modern Farmhouse", PHOTOS.farmhouse],
      ["Coastal", PHOTOS.coastal],
      ["Japandi", PHOTOS.japandi],
      ["Mid Century", PHOTOS.midcentury],
      ["Industrial", PHOTOS.industrial],
      ["Quiet Luxury", PHOTOS.luxury],
      ["Investor Neutral", PHOTOS.neutral],
      ["Florida Ranch", PHOTOS.ranch],
      ["Painted Brick", PHOTOS.paintedBrick],
      ["Resort Yard", PHOTOS.resortYard],
      ["Craftsman", PHOTOS.craftsman],
    ];
    document.getElementById("styleGrid").innerHTML = SL.slice(0, 4)
      .map(
        ([n, src], i) => `
  <div class="st"><div class="sw2" style="overflow:hidden">${photo(src, n + " interior design style")}</div><div class="nm">${n}<span>${String(i + 1).padStart(3, "0")}</span></div></div>`,
      )
      .join("");
    ...
    const P = [
      {
        ...
        f: [
          ...
          "Full style library, Reality Lock, keep/replace/remove controls",
          "Virtual staging, declutter, material swap, style transfer",
  ...
  <div class="plan ${p.pop ? "pop" : ""}"><h3>${p.n}</h3>
    ...
    <ul>${p.f.map((x) => `<li><i data-lucide="check"></i><span>${x}</span></li>`).join("")}${(
      ...
      .map((x) => `<li class="no"><i data-lucide="x"></i><span>${x}</span></li>`)
      .join("")}</ul></div>`,
      ).join("");
      document.querySelectorAll("#plans [data-plan]").forEach((a) =>
        a.addEventListener("click", (ev) => {
          const n = a.dataset.plan;
          track("plan_cta_clicked", { plan: n, billing: bill });
          if (n === "Free") return;
          ev.preventDefault();
          openCheckout(n);
        }),
    ...
    document.querySelectorAll("#billSeg button").forEach((b) =>
    ...
    function coRender(name) {
      ...
      document.getElementById("coTitle").textContent =
        `${p.n}, Billed ${bill === "yr" ? "Yearly" : "Monthly"}`;
    ...
    coMask.addEventListener("click", (e) => {
      if (e.target === coMask) closeCheckout();
    });
    ...
    (function foundingCount() {
      const left = document.getElementById("foundLeft"),
        big = document.getElementById("foundBig"),
        bar = document.getElementById("foundBar"),
        sub = document.getElementById("foundSub");
      if (!big) return;
    ...
    document.getElementById("faq").innerHTML = FAQ.map(
