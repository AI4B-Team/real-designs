ALTER TABLE public.presentations ADD COLUMN IF NOT EXISTS excluded_lines jsonb NOT NULL DEFAULT '[]'::jsonb;
...
CREATE OR REPLACE FUNCTION public.respond_to_presentation(_token text, _decision text, _note text DEFAULT NULL, _excluded jsonb DEFAULT '[]'::jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE st text; ex jsonb;
BEGIN
...
REVOKE ALL ON FUNCTION public.respond_to_presentation(text, text, text, jsonb) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_shared_presentation(text) FROM anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.respond_to_presentation(text, text, text, jsonb) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_shared_presentation(text) TO service_role;
GRANT EXECUTE ON FUNCTION public.respond_to_presentation(text, text, text, jsonb) TO service_role;