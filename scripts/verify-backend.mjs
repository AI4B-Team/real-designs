...
if (JSON.stringify(files.slice().sort()) !== JSON.stringify(sorted))
  fail("Migration filenames do not sort deterministically.");
...
for (const file of sorted) {
  ...
  if (/\blocalhost\b|127\.0\.0\.1|\bdev_only\b/i.test(stripped))
    fail(`${file}: environment-specific value in a migration.`);
  ...
  for (const m of stripped.matchAll(
    ...
    if (!re("GRANT[\\s\\S]*?ON\\s+(TABLE\\s+)?public\\.TBL"))
      fail(`${file}: public.${table} created without GRANT statements.`);
...
if (!staticOnly && process.env.PGHOST) {
  try {
    ...
    if (noPolicy)
      fail(
        `Tables granted to signed-in users but with no policy: ${noPolicy.split("\n").join(", ")}`,
      );
    ...
    if (Number(storagePolicies) < 12)
      fail(
        `Only ${storagePolicies} storage policies found; expected read/write/update/delete per bucket.`,
      );
